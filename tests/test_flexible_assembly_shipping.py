...
class FlexibleAssemblyShippingTests(AssemblyTask7TestCase):
    ...
    def test_browser_selection_recalculation_empty_state_and_stale_candidate_race(self):
        ...
        (async () => {
          ...
          await addButton.emit('click'); await pause();
          ...
          await form.emit('input',{target:quantity}); await pause();
          await nodes.recalculate.emit('click'); await pause();
          ...
          quantity.value='7'; await form.emit('input',{target:quantity}); await pause();
