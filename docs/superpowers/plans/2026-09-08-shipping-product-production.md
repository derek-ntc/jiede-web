...
- [x] 不屏蔽警告，执行完整 unittest 和完整差异检查；最终应用提交 `7fe4e14` 主控复跑 449 项、448 通过、1 既有跳过，44.928 秒。
- [x] 仅本机临时数据库、独立 cookie/密钥预览；实际验证规格、收货资料、组装删加、缺库确认、普通及组装发货、PDF、历史补价、生产客户筛选/工序和客户改名。完整同 token 网络重放由自动化事务/库存/文件测试验证，不混称浏览器操作证据。
- [x] 无价权限页面、JSON、Excel/PDF、登录/直接 PDF、新增 POST 的 CSRF/长度/来源边界由相应回归及实际流程覆盖。
- [x] 最终应用代码在两个全新升级副本上各初始化两次；原 39 表的原列/行保持不变，未自动补历史价格。
- [x] 整分支独立复核完成，5 项发现经统一修复及范围复查全部关闭；无剩余可操作问题。
- [x] 本地演示、使用说明和 PDF 检查结果已准备；保留隔离分支和原主目录，未合并、推送或部署。
- [ ] 桌面/窄屏整页视觉验收：内置浏览器视口受限，仍需用户本机确认；不影响已记录的功能流程和 PDF 验证结论。
...
- 当前阶段：Task 1–6 的隔离实施、自动化验收、本地使用说明及最终独立复核/修复复查已完成。桌面/窄屏整页视觉验收仍受工具限制；分支仅供本地验收，未合并、未推送、未部署，未修改正式数据。
